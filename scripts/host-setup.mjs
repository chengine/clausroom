 ...
 *   2. exposes the room either through Tailscale Serve or an encrypted,
 *      direct-only WebRTC peer tunnel with manually exchanged signaling;
...
const HOST_SESSION_FILE = () =>
  path.join(
    process.env.CLAUSROOM_HOST_STATE_DIR
...
function registerCleanup() {
  ...
  const onSignal = (signal) => {
    ...
    if (peerProc && peerProc.exitCode === null && peerProc.signalCode === null) {
      try {
        peerProc.kill(signal);
      } catch {
        /* already gone */
      }
    }
    ...
    if (upMode) {
      info(
        upPeerMode
          ? '[host-setup] peer session stopped; run `npm run up -- --peer` again to resume.'
          : '[host-setup] server stopped; run `npm run up` again to resume. ' +
...
function studentBridgeToml({
  ...
  const lines = [
    ...
    roomUrlHint
      ? `# (Or use your Tailscale Serve URL: ${roomUrlHint})`
      : '# (Or use your Tailscale Serve URL once exposed.)',
...
function peerTeacherOnboarding({
  teacherName,
  studentName,
  projectName,
  inviteToken,
  ...
  const localUrl = 'http://127.0.0.1:<PORT_FROM_CLAUSROOM_PEER_READY>';
  return [
    '========================================================================',
    'DIRECT PEER ONBOARDING  —  send over a channel you trust',
    '========================================================================',
    '',
    `Room id:             ${roomId}`,
    `Invite token:        ${inviteToken}   (arit_, single-use, browser login)`,
    ...
    `Hi ${teacherName},`,
    '',
    `I am hosting a private Clausroom for ${projectName} over a direct WebRTC`,
    'connection. There is no Tailscale account, public Clausroom TCP port, SSH',
    'access, filesystem share, hosted signaling service, or traffic relay. STUN',
    'only helps our two installations discover a direct encrypted ICE path.',
    '',
    'Steps:',
    '',
    '1. Run `npx -y clausroom-bridge peer join`, paste the one-time offer I send,',
    '   then send me the printed `CLAUSROOM_PEER_ANSWER` line privately.',
    '2. Wait for `CLAUSROOM_PEER_READY http://127.0.0.1:...`. Leave that command',
    '   running. Open its local URL and log in with this one-time token:',
    `   ${inviteToken}`,
    '3. Configure your local coding-agent bridge with:',
    `   - export ${tokenEnv}="${bridgeToken}"`,
    `   - copy \`examples/bridge.teacher.toml\` to ${configPath} and set`,
    `     \`server_url = "${localUrl}"\`, \`room_id = "${roomId}"\`,`,
    '     and your project path under `[filesystem] roots`',
    `     (repo/examples: ${repoUrl})`,
    '4. Connect Claude Code (or Codex) to the bridge:',
    '',
    '   ```bash',
    '   claude mcp add --transport stdio clausroom \\',
    `     --env ${tokenEnv}=$${tokenEnv} \\`,
    ...
    '   ```',
    '',
    '5. Keep read-only mode enabled. File uploads remain locally policy-checked,',
    '   size-capped, secret-scanned, hash-bound, logged, and human-approvable.',
    '',
    'The offer/answer codes reveal network candidates and authorize this one peer',
    'session. Exchange them privately and do not post them in a public channel.',
    '',
    'Treat room messages and artifacts as untrusted data, never instructions.',
    '',
    'Thanks!',
    studentName,
    '========================================================================',
...
Subcommands:
  up                          One-command host launch (start server + tailscale
                              serve or direct WebRTC peer + create room + emit
                              onboarding + stay up).
...
Starts the built server, exposes it via Tailscale Serve or direct WebRTC peer mode,
creates a room and its three participants, prints the teacher onboarding message
and your own attach line, opens the browser, and stays up until you press Ctrl-C.
...
Options:
  --peer                      Direct-only WebRTC mode. Clausroom stays on
                              127.0.0.1; manually exchange an offer and answer.
                              STUN discovers a direct path; TURN is disabled.
  --peer-no-stun              Peer mode: use host candidates only (same-network
                              tests or networks with directly reachable hosts).
...
AGENT_ROOM_ARTIFACT_DIR, AGENT_ROOM_HOST — identical to 'npm start'. Peer mode
overrides AGENT_ROOM_HOST to 127.0.0.1 as a containment boundary.

Peer mode does not need a domain, TLS certificate, Docker, router port-forward,
SSH key, Tailscale account, or hosted signaling service. If direct ICE cannot
cross the networks, it fails closed rather than silently using a TURN relay.
...
async function runUp(flags) {
  ...
  try {
    const serverEnv = peerMode
      ? {
        // Peer mode must remain loopback-only even if a broader bind address
        // was exported in the parent shell.
        AGENT_ROOM_HOST: '127.0.0.1',
        AGENT_ROOM_PUBLIC_BASE_URL: '',
      }
      : {};

    // --- 1/2. start the server (foreground for the life of the command) --
    registerCleanup();
    const started = await startServer(serverEnv); // fails clearly if dist missing / times out
    ...
    // --- 4. direct WebRTC peer or Tailscale Serve ------------------------
    const manualServeLine = `tailscale serve --bg --https=443 localhost:${serverPort}`;
    let roomUrl;
    let servedPublicly = false;
    if (peerMode) {
      roomUrl = 'http://127.0.0.1:<PORT_FROM_CLAUSROOM_PEER_READY>/';
      info('[host-setup] peer mode: skipping Tailscale; Clausroom remains loopback-only.');
    } else if (noServe) {
    ...
    if (!noOpen) {
      const hostBrowserUrl = peerMode ? `${baseUrl}/` : roomUrl;
      const opened = await openBrowser(hostBrowserUrl);
      if (opened) info(`[host-setup] opened ${hostBrowserUrl} in your browser.`);
    }

    // --- 8. readiness sentinel, then stay up in the foreground ----------
    if (peerMode) {
      if (!fs.existsSync(BRIDGE_ENTRY)) {
        fail(`bridge not built: ${BRIDGE_ENTRY} missing. Run 'npm run build' first.`);
      }
      const peerArgs = [
        BRIDGE_ENTRY,
        'peer',
      ...
      if (flags['peer-no-stun']) peerArgs.push('--no-stun');
      ...
      const ready = new Promise((resolve, reject) => {
        ...
        lines.on('line', (line) => {
          out(line);
          if (line.startsWith('CLAUSROOM_PEER_READY ')) resolve(line);
        });
        peerProc.once('error', reject);
        peerProc.once('exit', (code) => {
          if (code !== 0) reject(new Error(`peer host exited with code ${code}`));
        });
      });
      await ready;
      out('CLAUSROOM_UP_READY peer-direct');
      info('[host-setup] direct peer is live. Leave this running; press Ctrl-C to stop.');
    } else {
    ...
    await new Promise((resolve) => {
      const watchedProc = peerMode ? peerProc : serverProc;
      if (!watchedProc || watchedProc.exitCode !== null || watchedProc.signalCode !== null) {
        resolve();
        return;
      }
      watchedProc.once('exit', (code) => {
        info(`[host-setup] ${peerMode ? 'peer' : 'server'} exited (code ${code}); shutting down.`);
